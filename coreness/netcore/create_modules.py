...
def get_seed_modules(network,seed_nodes,random_walk_weights_pvals,pval_thresh,weight_thresh,max_subnet_size,vertex_index,output_file,output_dir):
	...
	if len(nodes_to_add)==0:
		print("No nodes were foud that could be added to the seed subnetwork!")
	...
	vertex_index_dict = dict(zip(vertex_index['node_index'], vertex_index['node']))
...
def plot_subet(subnet,seed_nodes,nodes_to_add,random_walk_weights_pvals,pval,weight,subnet_file):
	...
	nc = nx.draw_networkx_nodes(subnet,
                                    pos,
                                    nodelist=seed_nodes,
                                    node_color="darkorange",
                                    node_shape = "o",
                                    node_size=seed_sizes
        )
	...
	if len(nodes_to_add)>0:
		nc = nx.draw_networkx_nodes(subnet,
                                            ...
                                            node_color="darkgray",
                                            node_shape = "o",
                                            node_size=nodes_to_add_sizes
                )

	# add edges
	ec = nx.draw_networkx_edges(subnet,
                                    pos,
                                    restart_prob=0.5,
...
def plot_module(module,module_file,seed_nodes,vertex_index):
	...
	# set plotting params 
	nodes_labels = vertex_index.loc[vertex_index['node_index'].isin(nodes)]
	nodes_labels_dict = dict(zip(nodes_labels['node_index'],nodes_labels['node']))
...
def save_modules(modules,seed_nodes,random_walk_weights_pvals,vertex_index,output_file,output_dir):
	...
	with open(output_file,'w') as of:
		...
		for i, m in enumerate(modules):
			# get the nodes names 
			subnet_nodes = vertex_index.loc[vertex_index['node_index'].isin(m.nodes())]['node'].tolist()
