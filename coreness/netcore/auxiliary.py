...
def get_seed_nodes(seed_file,vertex_index):
	...
	return seed_list_index['node_index'].tolist()
...
def get_edges(edge_file,vertex_index=None):
	...
	if vertex_index is None:
		...
		all_nodes_unique_df = pd.DataFrame({"node":all_nodes_unique,"node_index":range(1,len(all_nodes_unique)+1)})
	else:
...
def get_vertex(vertex_file):
	...
	vertex_index.columns = ['node_index','node']
	vertex_index.loc[:,'node_index'] = vertex_index['node_index'].apply(lambda x: int(x))
...
def update_vertex_index(vertex_index,removed_nodes):
	...
	"""
	vertex_index_removed = vertex_index.loc[~vertex_index['node_index'].isin(removed_nodes)]
...
def get_weights(weights_file):
	...
	# TODO: instead of returning with error, remove NA values and contuine analysis
