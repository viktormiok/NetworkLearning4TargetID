...
def run(args):
	...
	if args.weights_file is None:
		...
		binary_weights = [1 if node in seed_nodes else 0 for node in vertex_index['node_index']]
	...
	# merge with vertex index to get the node name
	set_pvals_df_with_node_name = pd.merge(vertex_index,set_pvals_df,left_on='node_index',right_on='gene_index')
	set_pvals_df_with_node_name[['node_index','node','group','pvalue']].to_csv(args.output_dir+args.normalisaton_method+"_fold"+str(args.fold_num)+".txt",sep="\t",header=True,index=False)
