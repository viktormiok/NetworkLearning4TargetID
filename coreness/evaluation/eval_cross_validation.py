...
sys.path.append("/project/networkpropagation/NetCore/netcore/")
...
def get_class_vals(pval,fold_pvals,test_nodes,test_negatives):
	...
	under_thresh = fold_pvals[fold_pvals['pvalue']<pval]['node_index'].tolist()
...
def eval_fold(fold_num,cross_valid_dir,method,pval_thresh,all_negatives,G):
	...
	fold_pvals.node_index = fold_pvals.node_index.apply(int)

	# # convert old index to new index
	# fold_pvals_with_old_index = pd.merge(fold_pvals,vertex_index_with_old,left_on="node_index",right_on="old_index")

	# get the training and test sets
	test_nodes = fold_pvals[fold_pvals['group']=="test"]['node_index'].tolist()
...
def plot_all_methods(methods,methods_fold_eval,cross_valid_dir):
	...
	pyplot.legend(title='Method - AUPRC', labels=methods_auc["method"] + " " + round(methods_auc['auprc'],3).map(str),fontsize=20)

	fig_file=cross_valid_dir+"method_compare_pre_rec.pdf"
	pyplot.savefig(fig_file,dpi=300)
	pyplot.close()
	...
	ax = sns.lineplot(data=methods_fold_eval_means,
				 ...
				 palette=sns.color_palette("Set2",len(methods)),style="method",dashes=False,
				 markers=True)
	ax.set_xlabel("FPR",fontsize=28)
	ax.set_ylabel("Recall",fontsize=28)
	ax.tick_params(labelsize=20)
	pyplot.legend(title='Method - AUROC', labels=methods_auc["method"] + " " + round(methods_auc['auroc'],3).map(str),fontsize=20)
...
def run(args):
	...
	# # using older index because of cross valid results
	# old_vertex_index=pd.read_csv("/project/networkpropagation/NetCore/networks/CPDB/CPDB_0.95_index_vertex.txt",header=None,sep="\t")
	# old_vertex_index.columns = ['old_index','node']
	# vertex_index_with_old = pd.merge(vertex_index,old_vertex_index,on="node")
